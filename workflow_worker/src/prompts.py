...
def genai_use_cases_system_prompt(
    ...
    return (
        ...
        "The three candidates must be meaningfully varied: do not submit three "
        "versions of the same workflow, user group, data asset, business problem, "
        "or generic GenAI pattern. Cover different angles such as practical "
        "operational value, bold strategic advantage, and surprising "
        "company-specific invention when the evidence supports them.\n"
        ...
        "and what makes the solution distinctive. Each why_iconic field must "
        "explain why the use case is memorable, company-specific, and high-impact, "
        "not merely useful. Link every candidate to at least one pain point title "
        "from the input.\n"
...
def use_case_grader_system_prompt() -> str:
    return (
        ...
        "Be harsh and discriminate between iconic company-specific ideas and merely "
        "competent ones. Use the full 1-10 scale instead of clustering scores near "
        "the top. Ordinary acceptable ideas should land around 4-6, strong ideas "
        "around 7-8, and scores of 9-10 should be rare and reserved for exceptional "
        "use cases with unusually strong distinctiveness, impact, GenAI fit, and "
        "grounding.\n"
        "Score each rubric dimension from 1 (weak) to 10 (exceptional):\n"
        "- company_relevance: factual fit with this company's business lines, "
        "priorities, users, pain points, context, and available evidence. Treat "
        "this as a grounding check, not the main differentiator; a highly relevant "
        "but ordinary idea should still lose to a more iconic, high-impact one.\n"
        "- business_impact: expected operational, financial, customer, risk, or "
        "strategic value if executed well.\n"
        "- iconicness: the decisive criterion. Score whether this feels distinctive, "
        "memorable, strategically meaningful, and like something this company in "
        "particular would be proud to put its name on, not a default GenAI idea.\n"
        ...
        "Do not let generic but relevant use cases win. A use case can score high "
        "overall only if iconicness is also high; company relevance should confirm "
        "the fit, while iconicness should separate the winners.\n"
